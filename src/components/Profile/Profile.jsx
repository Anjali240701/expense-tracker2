...
const Profile = () => {
  ...
  useEffect(() => {
    fetch(
      "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyBXLa4m2AbfwfjEEoi8ijV2wWJvFgJjeOU",
      {
        method: "POST",
    ...
    if (enteredName.length > 0 && enteredPhotoUrl.length > 0) {
      try {
        const response = await fetch(
          "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyBXLa4m2AbfwfjEEoi8ijV2wWJvFgJjeOU",
          {
            method: "POST",
            body: JSON.stringify({
              idToken: token,
  ...
  const verifyEmailHandler = async () => {
    try {
      const response = await fetch(
        "https://identitytoolkit.googleapis.com/v1/accounts:sendOobCode?key=AIzaSyBXLa4m2AbfwfjEEoi8ijV2wWJvFgJjeOU",
        {
          method: "POST",
          body: JSON.stringify({
  ...
  return (
    <section className={classes.profile}>
      ...
      <div className={classes.user}>
        ...
        <p><img src={userState.profilePhotoUrl}/></p>
        <p>{userState.email}</p>
