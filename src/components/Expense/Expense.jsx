...
const Expense = (props) => {
    ...
    const fetchExpenses = async () => {
      try {
        const response = await fetch(
          `https://signup-141da-default-rtdb.firebaseio.com/${email}/expenses.json`
        );
        if (!response.ok) {
  ...
  const expenseDeleteHandler = async (id) => {
    try {
      const response = await fetch(
        `https://signup-141da-default-rtdb.firebaseio.com/${email}/expenses/${id}.json`,
        {
          method: "DELETE",
        }
      );
  ...
  const updateExpense = async (expense) => {
    try {
      const response = await fetch(
        `https://signup-141da-default-rtdb.firebaseio.com/${email}/expenses/${idEditSelected}.json`,
        {
          method: "PUT",
          body: JSON.stringify(expense),
          headers: {
  ...
  return (
    <>
      ...
      <section className={classes.expense}>
        ...
        <div>
          <h3> Total Money Spent :{totalMoneySpent}</h3>
